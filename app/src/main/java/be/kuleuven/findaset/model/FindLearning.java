...
public class FindLearning extends AbstractFindASet{
    ...
    public void startNewGame() {
        ...
        learningActivity.notifyNewGame(12);
        win = false;
    }
        ...
        for (int col = 0; col < 4; col++) {
            if (featureMatrix[0][col] == featureMatrix[1][col]) {
                if (featureMatrix[1][col] != featureMatrix[2][col]) {
                    ...
                    learningActivity.notifyFeatureSame(col);
                }
            } else {
                if (featureMatrix[0][col] + featureMatrix[1][col] + featureMatrix[2][col] != 6) {
                    ...
                    learningActivity.notifyFeatureDifferent(col);
                }
            }
        }
        return isSet;
    ...
    public void updateTable(ArrayList<Integer> selectedCardsIndex) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (foundedSetCardsIds.size() <= 69) {
                ...
                // 4.
                learningActivity.notifyCard(selectedIndex);
            }
        }
        if (foundedSetCardsIds.size() <= 69) {
            ...
            for (int j = 9; j < 12; j++) {
                learningActivity.notifyUnavailable(j);
            }
            initializeTable(9);
            justForTest = new int[3];
            alternativeSetCardsTable(9);
            learningActivity.notifyNewGame(9);
            if (!checkAllSetOnPage()) {
                win = true;
            }
        } else if (foundedSetCardsIds.size() == 75) {
            for (int k = 6; k < 9; k++) {
                learningActivity.notifyUnavailable(k);
            }
            initializeTable(6);
            justForTest = new int[3];
            alternativeSetCardsTable(6);
            learningActivity.notifyNewGame(6);
            if (!checkAllSetOnPage()) {
                win = true;
            }
        }
        if (win) {
            for (int l = 0; l < cardsIdTable.size(); l++) {
                learningActivity.notifyUnavailable(l);
            }
            learningActivity.setTestTxt("YOU WIN!!!");
        }
    }
    ...
    private boolean checkAllSetOnPage() {
        ...
        learningActivity.setTestTxt(testMessage.toString());
        ...
        if (!isSelected) {
            ...
            if (selectedCardsIndex.size() == 3) {
                if (checkSet(selectedCardsIndex)) {
                    learningActivity.setTestTxt("set Found");
                    this.updateTable(selectedCardsIndex);
                    learningActivity.notifyFeatureBox();
    ...
    public void select(int index) {
        ...
        learningActivity.notifySelect(index);
    }

    @Override
    public void unselect(int index) {
        isCardSelected.set(index, false);
        learningActivity.notifyUnselect(index);
    }
